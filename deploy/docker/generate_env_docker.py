...
def main() -> int:
    ...
    merged.update(
        {
            ...
            "DASHSCOPE_API_KEY": merged.get("DASHSCOPE_API_KEY") or merged.get("AGENT_LLM_API_KEY", ""),
            "MULTI_VIEW_PROMPT_MODEL": merged.get("MULTI_VIEW_PROMPT_MODEL", "qwen3-vl-plus"),
            "MULTI_VIEW_PROMPT_THINKING_BUDGET": merged.get("MULTI_VIEW_PROMPT_THINKING_BUDGET", "81920"),
            ...
            "CLOSEAI_TIMEOUT_SECONDS": merged.get("CLOSEAI_TIMEOUT_SECONDS", "1200"),
            "AI_MAX_FUSION_IMAGES": merged.get("AI_MAX_FUSION_IMAGES", "6"),
            "QUEUE_NAME": merged.get("QUEUE_NAME", "jinma-ai"),
            "WORKER_CONCURRENCY": merged.get("WORKER_CONCURRENCY", "2"),
            "QUEUE_JOB_TIMEOUT_SECONDS": merged.get("QUEUE_JOB_TIMEOUT_SECONDS", "1500"),
    ...
    lines = [
        ...
        "",
        "# Queue Worker 进程数量。每个 worker 同一时间执行 1 个队列任务。",
        f"WORKER_CONCURRENCY={merged['WORKER_CONCURRENCY']}",
        "",
        ...
        "# 多视图提示词反推配置。DASHSCOPE_API_KEY 留空时复用 AGENT_LLM_API_KEY。",
        f"DASHSCOPE_API_KEY={merged['DASHSCOPE_API_KEY']}",
        f"MULTI_VIEW_PROMPT_MODEL={merged['MULTI_VIEW_PROMPT_MODEL']}",
        f"MULTI_VIEW_PROMPT_THINKING_BUDGET={merged['MULTI_VIEW_PROMPT_THINKING_BUDGET']}",
